...
function AdminTurf(){
 ...
 return <>
   ...
   <div className="settings-grid-v2">
     <SettingCard eyebrow="PUBLIC PROFILE" title="Turf identity" description="The core information visitors see on the public Bason Turf City page."><div className="form setting-form"><label>Turf name<input value={displayBrand(turf.name)} onChange={e=>set('name',e.target.value)}/></label><label>Owner name<input value={turf.ownerName||''} onChange={e=>set('ownerName',e.target.value)}/></label><label>Turf size<input value={turf.turfSize||''} onChange={e=>set('turfSize',e.target.value)}/></label><label>Turf type / surface<input value={turf.turfType||''} onChange={e=>set('turfType',e.target.value)}/></label><label className="full-span">Description<textarea value={turf.description||''} onChange={e=>set('description',e.target.value)}/></label><label className="full-span">Additional information<textarea value={turf.additionalInfo||''} onChange={e=>set('additionalInfo',e.target.value)}/></label></div></SettingCard>
