...
function AdminTurf(){
 const {isAdmin,loading:roleLoading,uid,email,can}=useAdminRole();
 const canManage=can('manageTurfSettings');
 ...
 const save=async()=>{
  if(!canManage){
    alert(`Your account does not have permission to manage Turf Settings. Ask the project owner to assign an appropriate Admin access level to your Firebase user.`);
    return;
  }
  ...
  try{
    ...
    if(x?.code==='permission-denied'){
      alert('Your account is not authorized to update Turf Settings. Please verify your Admin access level and permissions.');
    }else{
 ...
 return <>
   {!canManage&&<div className="error" style={{marginBottom:16}}>
     Your Firebase account is signed in, but it is not registered as an Admin. The current user is <b>{email||'unknown'}</b>{uid?<> (UID: <code>{uid}</code>)</>:null}. Ask the project owner to assign an Admin access level to your account.
   </div>}
   <AdminPageHeader eyebrow="TURF MANAGEMENT" title="Turf settings" subtitle="Keep every public-facing turf detail and booking rule in one organized workspace." actions={<button className="primary" onClick={save} disabled={saving || !canManage}><Save/> {saving?'Saving…':'Save changes'}</button>}/>
   <div className="settings-grid-v2">
     <SettingCard eyebrow="PUBLIC PROFILE" title="Turf identity" description="The core information visitors see on the public TestWeb Turf page."><div className="form setting-form"><label>Turf name<input value={displayBrand(turf.name)} onChange={e=>set('name',e.target.value)}/></label><label>Owner name<input value={turf.ownerName||''} onChange={e=>set('ownerName',e.target.value)}/></label><label>Turf size<input value={turf.turfSize||''} onChange={e=>set('turfSize',e.target.value)}/></label><label>Turf type / surface<input value={turf.turfType||''} onChange={e=>set('turfType',e.target.value)}/></label><label className="full-span">Description<textarea value={turf.description||''} onChange={e=>set('description',e.target.value)}/></label><label className="full-span">Additional information<textarea value={turf.additionalInfo||''} onChange={e=>set('additionalInfo',e.target.value)}/></label></div></SettingCard>
