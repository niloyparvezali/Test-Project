...
import { localDate,money } from '../../utils/dateUtils';
...
function Expenses(){
 ...
 const todayTotal=expenses.filter(x=>x.date===today).reduce((a,x)=>a+Number(x.amount||0),0),monthTotal=expenses.filter(x=>String(x.date||'').startsWith(month)).reduce((a,x)=>a+Number(x.amount||0),0),yearTotal=expenses.filter(x=>String(x.date||'').startsWith(year)).reduce((a,x)=>a+Number(x.amount||0),0);
 const rows=expenses.filter(x=>`${x.reason} ${x.category} ${x.note}`.toLowerCase().includes(q.toLowerCase())).sort((a,b)=>String(b.date).localeCompare(String(a.date)));
 return <>
   <AdminPageHeader eyebrow="EXPENSES" title="Operating costs" subtitle="Record and review the costs that keep the turf running." actions={<button className="primary" onClick={()=>setModal({})}><Plus/> Add expense</button>}/>
